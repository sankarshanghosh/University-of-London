function PayGapByJob2017() {
  ...
  this.draw = function () {
    ...
    for (i = 0; i < this.data.getRowCount(); i++) {
      var proportionFemale = this.data.getNum(i, 'proportion_female');
      var payGap = this.data.getNum(i, 'pay_gap');
      var numJobs = this.data.getNum(i, 'num_jobs');
  
      // Map the data to pixel values
      var x = map(proportionFemale, 0, 100, this.pad, width - this.pad);
      var y = map(payGap, -20, 20, height - this.pad, this.pad);
      var size = map(numJobs, 109, 2605, this.dotSizeMin, this.dotSizeMax);
